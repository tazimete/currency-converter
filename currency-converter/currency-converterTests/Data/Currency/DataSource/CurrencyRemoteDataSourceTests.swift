...
class CurrencyRemoteDataSourceTests: XCTestCase {
    ...
    func testGetWithSuccessResponse() {
        let expectation = self.expectation(description: "Wait for currency remote data source to load for success response...")
        let amount = "45875"
        let currency = "JPY"
    ...
    func testGetWithErrorResponse() {
        ...
        currencyRemoteDataSource = MockRemoteCurrencyDataSource(apiClient: MockAPIClient(session: MockURLSessionFailed(configuration: URLSessionConfigHolder.config)))
